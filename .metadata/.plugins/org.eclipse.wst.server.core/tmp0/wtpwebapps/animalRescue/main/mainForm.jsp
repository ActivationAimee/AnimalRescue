			...
			<div class="wrap-list">
					<ul>
						<li class="notice-title bgblur">
							<a href="/animalRescue/SearchList.anires">
						...
						<li class="notice1">
							<a href="/animalRescue/adopt_list.anires">
								<p class="txt20">買わないで<br>里親がなるのは。。</p>
								<p class="txt17">里親申請案内</p>
							</a>
						</li>
						<li class="notice2">
							<a href="/animalRescue/list.anires?page=1">
								<p class="txt20">我が子が痛くて<br>病気があります。</p>
								<p class="txt17">病院探し</p>
							</a>
						</li>
						<li class="notice3">
							<a href="/animalRescue/protectList.anires?page=1">
										...
										<% 
											...
											for(int i=0; i<4;i++){
												if(ani_no2!=0){
													...
													if(aa!=null){
										%>
										<li><p><%= aa.getAni_date().substring(2) %></p><a href="javascript:void(window.open('/animalRescue/adopt_select.anires?ani_no=<%=ani_no2%>','<%=aa.getAni_subject()%>'))" class="color-sub">
							...
							<div>
								<h2 class="anounce">サポート掲示板<a href="/animalRescue/supportboard.anires" class="more">more</a></h2>
									<ul>
										...
										%>
										<li>
										    <p><%=st.getDate().substring(2,10) %></p>
										    <a href="/animalRescue/supportView.anires?snum=<%=ani_no3%>" class="color-sub">
										     <%=st.getSubject()%><span class="new">N</span></a></li>
										<%
										ani_no3--;
										}else break;
												}} %>
