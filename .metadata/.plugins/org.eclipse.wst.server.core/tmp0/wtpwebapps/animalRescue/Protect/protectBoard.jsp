	...
	<body>
		...
		<div class="topnav">
		  <div class="search-container">
		    <form method="post" action="protectSearch.anires">
		     <select name='col'>
		      	<option value='area'>地域</option>
		        <option value='name'>保護センター名</option>
		      </select>
		...
		<div class = "mid_content">
			...
			<table>
				<tr>
					<th>地域</th>
					<th width="50%">保護センター名</th>
					<th>電話番号</th>
				</tr>
				<c:forEach items = "${ptList}" var = "pt">
					<tr onclick = "location.href = 'protectClick.anires?num=${pt.protectnum}'" style="cursor:hand" class="listbar">
							<td>${pt.area}</a></td>
							<td>${pt.name}</td>
							<td>${pt.tel}</td>
					</tr>
			...
			<div style ="text-aling : 'center'">
				<c:forEach items = "${page}" var = "pagenum" varStatus="status">
						<a href ="protectList.anires?page=${status.index+1 }">${status.index+1 }</a>&nbsp;
				</c:forEach>
			</div>
			 <p align="right">
			 <c:if test = "${member.admin==1 }">
				<button class="btn_ok" onclick = "location.href='/saveTheAnimal/Protect/protectWrite.jsp'">登録</button>
			</c:if>
				<button class="btn_ok" onclick = "location.href='/animalRescue/protectMap.anires'">地図</button>
			</p>
		</div>
