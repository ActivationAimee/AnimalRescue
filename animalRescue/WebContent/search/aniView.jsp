							...
							<c:choose>
								<c:when test = "${member.admin==1 }">
									<c:if test = "${aniView.ani_now==0 }">
										<input type = "button" value="修正" class = "btn_ok" onclick = "location.href='/animalRescue/searchModify.anires?ani_no=${aniView.ani_no }'"/>
									</c:if>
										<input type = "button" value ="削除" class = "btn_ok" onclick = "location.href='/animalRescue/searchDelete.anires?ani_no=${aniView.ani_no }'"/>
								</c:when>
								<c:when test = "${member.admin==0 }">
									...
									<c:if test = "${aniView.ani_attribute==1 && aniView.ani_now==0 && member!=null}">
										<input type = "button" value="里親申請" class = "btn_ok" onclick = "javascript:void(window.open('adopt_writeForm.anires?ani_w=${aniView.ani_no }'))"/>
									</c:if>
								</c:when>
							</c:choose>
							<input type="button"class = "btn_ok" value = "戻る" onclick = "history.back()"/>
						</th>
					</tr>
				</table>
			</div>
